	...
	try{
		...
		if(isset($_POST['add-appointment'])){
			...
			if($appointment->checkingPatientCard($card_number)){
				...
				$_SESSION['error'] = "You Have Added An Appointment For $$patient_name On This Date $dateof_appointment Before";
				$all_purpose->redirect("schedule-patient-appointment.php");
			}else{
				if($appointment->addingPatientAppointment($staff_number, $patient_number, $dateof_appointment, $unit_id)){
					...
					$_SESSION['success']= "You Have Added an Appointment For $patient_name On Date 
			...
			$_SESSION['error'] = "Please Fill The Below Form To Schedule Appointment For The Patient";
			$all_purpose->redirect("schedule-patient-appointment.php");
		}
